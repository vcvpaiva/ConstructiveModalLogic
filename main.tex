...
Each of the logics presented in this section are extensions of the
single-conclusion formalization of Gentzen's  LJ which produces  the
intuitionistic sequent calculus.  The syntax of formulas for LJ is
...
initial system.  The set of initial inference rules are as follows:
\begin{center}
  \small
  \begin{math}
    \begin{array}{c}
...
CS4.  This formalization was initially denoted by IS4 (Intuitionistic modal logic S4) and was proposed by
Bierman and de Paiva \cite{CS4}.  Since  Simpson's systems,  framework starting from intuitionistic K,  were also called IK, IS4, IS5, it made more sense to change the name of Bierman and de Paiva's system to CS4.  
...
This systemem is very well behaved proof theoretically. We can show that it has a Hilbert-style presentation, and a Natural Deduction one, as well as a sequent formulation. The sequent calculus satisfies cut-elimination, an old result from Ohnishi and Matsumoto \cite{} as well as the subformula property. The Natural Deduction formulation has a  colourful history, described in \cite{CS4}. One of its distinct features is that it was described in Prawitz' seminal book in Natural Deduction \cite{prawitz1965}, hence it is still called sometimes Prawitz modal logic.

Most interestingly the system has both Kripke and categorical semantics, described  respectively in \cite{alechinaetal} and \cite{CS4}. Since we can prove a Curry-Howard correspondence for this system, it has been used in several applications within programming language theory. Examples include \cite{hmm}.

\subsection{Ewald's Temporal Logic}

Ewald proposed an intuitionistic temporal logic where the traditional
modal operators, $\Box$ and $\Diamond$, have been decomposed into more
canonical modal operators he calls $\F$, $\H$, $\G$, and $\P$
\cite{ewald1986}.  The sequent calculus formalization of Ewald's
...
\begin{center}
  ...
  \end{math}
\end{center}
Looking at Ewald's system in the present with a categorical eye we can
now see that these new operators are adjoint functors
\cite{Menni:2014}.  The rules $\text{G}$ and $\text{H}$ show that $\G$
...
$\text{GP}$ give the co-unit and unit of the adjunction $P \dashv G$.

The following table describes the intuitive meaning of each of Ewald's
operators:
