...
\[ \bfig
  ...
  A monoidal category can be symmetric or commutative (meaning $A\times B$ is isomorphic to $B\times A$) or not. If it is symmetric we call the isomorphism $\beta\colon A\times B\to B\times A$, for a \textit{braid}. 
...
A monoidal adjunction $J \dashv H, $ is an adjunction  where both functors are monoidal and the unit, and co-unit are monoidal natural transformations too. Finally,
a monoidal comonad is a comonad $(\Box,\varepsilon, \delta)$ on a monoidal category $\cat{C}$, with extra structure preserving the monoidal structure. Thus $\Box\colon \cat{C}\to \cat{C}$ satisfies the following diagrams.{\todo{add diagrams here}}
...
\begin{definition}
  \label{def:SMCADJ}
  A \textbf{symmetric monoidal adjunction} between two SMCs
  $\cat{M}_1$ and $\cat{M}_2$ is an ordinary adjunction where the
  adjoint functors are symmetric monoidal functors, and the unit and
  counit are symmetric monoidal natural transformations.
