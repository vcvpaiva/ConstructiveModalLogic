...
\begin{proof}
  ...
  in Definition~\ref{def:CS4-single-adjoint-cat-model} such that $J
  \dashv H$.  Furthermore, define $\Box = HJ$ and $\Diamond = JH$.  It
  is a well-known fact about adjoints that $\Box$ is a comonad and
  $\Diamond$ is a monad.  By definition we assumed that $\Box$ is
  monoidal, but we must show that $\Diamond$ is also monoidal.
