...
This will require  some categorical definitions that are not very  traditional ones, hence  these are provided in the appendix A, but discussed briefly here.  We will need the categorical concepts of 
\begin{enumerate}
    ...
    \item {(symmetric) monoidal (co)monad.} 
%    \item {monoidal monad.}
\end{enumerate}  
%which we describe in the appendix A.

%\subsection{Monoidal notions}
A  \textbf{monoidal} category  is simply a category $\cat{C}$  equipped with a tensor product $\otimes$ structure $(\otimes, I, \alpha, \rho, \lambda)$,  a generalization of traditional categorical products, satisfying two coherence conditions: MacLane's pentagon condition relating the associativity $\alpha$  of tensor to its bifunctorial structure, and the triangle condition relating the unit of the tensor $I$ to its natural transformations $\rho$ and $\lambda$,  plus the unit equation $\rho_I=\lambda_I$.
    
  A monoidal category can be \textbf{symmetric}, meaning that the tensor product $A\otimes B$ is isomorphic to $B\otimes A$, or not. If the monoidal category is symmetric we call $\beta$ (for  \textit{braid}) the isomorphism going from $ A\otimes B$ to $B\otimes A$. 
  Commutativity of the tensor product brings along  three new diagrams (see definition 6 %\label{def:monoidal-category}
in the appendix A).
...
A \textbf{(symmetric monoidal) functor} between two (symmetric) monoidal categories $\cat{C}$ and $\cat{D}$ is simply a functor $F\colon \cat{C} \mto \cat{D}$ that relates the monoidal structures, that is, there are (comparison) natural transformations  $F(A)\otimes F(B)\mto F(A \otimes B)$, for each $A,B$ in $\cat{C}$ (as well as a map $m_{I}\colon I \mto FI$) that interact well with the monoidal structures  in $\cat{C}$ and  $\cat{D}$ (older books insist on an isomorphism between the tensor structures, we only require coherent natural transformations). 
%This means another three commuting diagrams, one for associativity, one for left, one for right isos. Since the tensor product we consider is symmetric, one more commuting diagram is neccessary, preserving $\beta$, see definition 8 %\label{def:SMCFUN}  in the appendix.

Given monoidal functors $(F,m)$ and $(G,n)$ a \textbf{symmetric monoidal natural transformation} from $F$ to $G$ is a natural transformation which is compatible with the comparison maps $m$ and $n$, which means the two extra diagrams in definition 9 in the appendix commute too.

A \textbf{(symmetric) monoidal adjunction} $F \dashv G$ is an adjunction  where both functors $F,G$  are (symmetric) monoidal functors and the unit, and the co-unit are (symmetric) monoidal natural transformations. 

Lastly, a \textbf{(symmetric) monoidal comonad} is a comonad, let us write it directly as $(\Box,\varepsilon, \delta, m)$ on a monoidal category $\cat{C}$, with extra structure preserving the monoidal structure of the category. Thus $\Box\colon \cat{C}\to \cat{C}$ satisfies the  diagrams in definition 11 in appendix A.
%{\todo{add diagrams there}}
Similarly for a monoidal monad.

Since any cartesian closed category is a (symmetric) monoidal closed category, which simplifications of the structures described do we get? Surely the results corresponding to Benton's  in \cite{benton1995} should remain true, but do we get anything else? We discuss this in the next section.





\subsection{Cartesian Closed Adjunctions}
\label{subsec:cartesian_closed_adjunctions}
Cartesian closed categories are particular types of symmetric monoidal
closed categories where the tensor product is the cartesian product.

The following lemma summarizes the symmetric monoidal structure in a cartesian closed category, its proof is well known.
...
As we can see from the previous definition cartesian products are much more constrained than arbitrary tensor products.  We can capitalize on this fact
and obtain a simplified notion of symmetric monoidal functors between
...
\fi
\noindent
However, it turns out that any product functor is a strong.
...
\fi
\noindent
A more surprising result is that any strong symmetric monoidal functor
between cartesian closed categories is a product functor.
...
Benton showed that the left adjoint in any symmetric monoidal
adjunction between a cartesian closed category and a symmetric
monoidal closed category is strong \cite{benton1995}, but this result
can be generalized to arbitrary symmetric monoidal categories.
...
Finally, using the previous result we can show that the left adjoint
of any symmetric monoidal adjunction between cartesian closed
categories is a product functor.  In addition, it turns out that we
can exploit the fact that right adjoints preserve limits to obtain
that both functors in the adjunction are in fact product functors.
...
\begin{proof}
  ...
  it is strong.
\end{proof}
% subsection cartesian_closed_adjunctions (end)

\subsection{Modeling Modal Systems}
Now to model a constructive modal system like CS4 we first recall that
to model simply intuitionistic propositions we need a category
$\cat{C}$ with products, coproducts and internal-homs, to model,
respectively conjunctions, disjunctions and logic implications.

Thus we recap that to model a necessity-like S4 operator we will need
a (symmetric monoidal) comonad $(\Box,\varepsilon, \delta, m)$ on the
cartesian closed category $\cat{C}$.
...
Finally, the interesting ones. The equations that mix the comonad with the monad structure:
