...
Now the existence of any (monoidal or not) adjunction provides us with a (monoidal) monad $HJ$ in $\cat{C}$, as well as a (monoidal) comonad $\Box=HJ$ on $\cat{C}$. Which conditions do we need in the adjunction to make this induced monad $\Box$-strong, if any?
...
\begin{proof}
  ...
  \end{array}
  \]
  Thus, the following diagrams commute (proofs omitted):
  \begin{mathpar}
    \bfig
    \hSquares|ammmmaa|/->``->`->``->`/[
      \Diamond^3 A`
