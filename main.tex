...
\begin{proof}
  ...
  $\Box = HJ$ and $\Diamond = JH$. Furthermore, suppose $J \dashv H$,
  $\varepsilon : \Box A \to A$, and $\eta : A \to \Diamond A$.  It is
  a well-known fact about adjoints that $(\Box, \varepsilon, \delta)$,
  where $\delta : \Box A \to \Box\Box A$ is a comonad, and $(\Diamond,
  \eta, \mu)$, where $\mu : \Diamond\Diamond A \to \Diamond A$ is a
  monad.  By definition we assumed that $\Box$ is monoidal, but we
  must show that $\Diamond$ is also monoidal.

  We know that both $(H,n)$ and $(J,m)$ are monoidal endofunctors on
  $\cat{C}$ which implies that their composition $\Diamond$ is
  monoidal where
  \[
  \begin{array}{lll}
    \mathsf{p}_{\top} = \eta_{\top} : A \to \Diamond A\\
  ...
  \begin{center}
    \begin{math}
      \begin{array}{cccc}
        ...
        \begin{array}{lll}
          \bfig
          \hSquares|ammmmaa|/->``->`<-``->`/[A`B``C`D`;f``h`g``i`]
          \efig\\
          \bfig
          \hSquares|ammmmaa|/->``->`<-``->`/[A`B``C`D`;f``h`g``i`]
          \efig
  ...
  Using this we can also define the $\Box$-strength map as follows:
  \[
  ...
  %% the triangle  
  At this point we must show that all of the appropriate diagrams
  given in Definition~\ref{def:comonad-strong-monad} commute.
\end{proof}
