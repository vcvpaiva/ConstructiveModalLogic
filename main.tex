...
\todo{Threw this in a subsection for now, we can move it around, and
  reword as needed.}

The definitions in this section depend on the definitions of the
categorical concepts of monoidal category, monoidal functor, and
monoidal adjunction.  Due to space limitations we omit their
definitions see \cite{benton1995} for their full definitions.
Similarly, for the definitions of monoidal comonad and monad see
\cite{CS4}.

\begin{definition}
  \label{def:comonad-strong-monad}
  Suppose $(\Box, \epsilon, \delta, \m{A,B}, \m{I})$ is a monoidal
  comonad on a cartesian closed category $\cat{C}$.  Then a
  \emph{$\Box$-strong monad} is a monad $(\Diamond,\eta,\mu)$ on
  $\cat{C}$ such that there exists a natural transformation:
  \[
...
\begin{definition}
  ...
  \item A pair of monoidal functors, $(J,n) : \cat{C} \to \cat{C}$ and
    $(H,m) : \cat{C} \to \cat{C}$, which form a monoidal adjunction
    with $J \dashv H$, and
  \item A HJ-strong monad $(\Diamond, \eta, \mu, \st{A}{B})$.
  \end{enumerate}
\end{definition}
