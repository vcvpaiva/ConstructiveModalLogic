...
Now to model a constructive modal system like CS4 we first recall that to model simply intuitionistic propositions we need a category  $\cat{C}$ with products, coproducts and internal-homs, to model, respectively conjunctions, disjunctions and logic implications. Now in appendix B we dscussed

Then we recap that to model a necessity-like S4 operator we will need a (symmetric monoidal) comonad $(\Box,\varepsilon, \delta, m)$ on $\cat{C}$.
